...
function Chatbot() {
    ...
    useEffect(() => {
        ...
        // Chỉ fetch + show welcome khi user thay đổi (login mới hoặc login lại)
        if (prevUserIdRef.current === dataUser._id) return;
        prevUserIdRef.current = dataUser._id;

        const fetchMessageChatbot = async () => {
            try {
                const res = await requestGetMessageChatbot();
                const history = res.metadata || [];
    ...
    const handleSend = async () => {
        ...
        try {
            ...
            const { answer, suggestedProducts } = res.metadata;
    ...
    return (
        <div className="fixed bottom-30 right-6 z-50">
            {isOpen ? (
                <div className="bg-white rounded-2xl shadow-2xl w-[600px] max-w-[calc(100vw-2rem)] h-[650px] flex flex-col border border-gray-100 overflow-hidden">
                    {/* Header */}
                            ...
                            >
                                <div
                                    className={`flex items-start gap-2 ${
                                        message.sender === 'user' ? 'flex-row-reverse max-w-[80%]' : 'w-full'
                                    }`}
                                >
                                    {message.sender === 'bot' && (
                                        <img
                                            ...
                                            <div className="mt-2 flex flex-col gap-2">
                                                {message.suggestedProducts.map((product) => (
                                                    <Link
                                                        ...
                                                        className="flex items-center gap-2 bg-white border border-purple-200 rounded-xl px-3 py-2 shadow-sm hover:border-purple-400 hover:shadow-md transition-all no-underline"
                                                    >
                                                        {product.image && (
                                                            <img
                                                                src={`${import.meta.env.VITE_URL_IMAGE}/uploads/products/${product.image}`}
                                                                alt={product.name}
                                                        ...
                                                        <span className="text-xs text-purple-500 flex-shrink-0">Xem →</span>
                                                    </Link>
                                                ))}
                                            </div>
                                        )}
